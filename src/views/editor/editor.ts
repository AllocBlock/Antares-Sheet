...
export class SheetEditor {
    ...
    loadSheet(sheetText: string, clearHistory : boolean) {
        ...
        this.meta.assign(meta)
        this.core.replaceSheet(root, true, clearHistory)
    ...
    loadSheetFromFile(clearHistory : boolean) {
        ...
        input.onchange = e => {
            ...
            fileReader.onload = () => this.loadSheet(fileReader.result as string, clearHistory)
    ...
    exportSheetToFile(attachedChords: FretChord[], fileName: string) {
        let sheetText = this.getSheetString(attachedChords.map(fretChord => fretChord.chord))
    ...
    removeUnderline(node: SheetNode) {
        this.core.removeUnderlineFromChordToNext(node);
        NodeUtils.validateSheetTree(this.root)
    }
    ...
    setSheet(meta: SheetMeta, root: SheetNode) {
        ...
        this.core.replaceSheet(root, true, false)
    }
}